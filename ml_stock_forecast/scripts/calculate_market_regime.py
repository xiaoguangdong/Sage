...
class MarketRegimeAnalyzer:
    ...
    def calculate_features(self):
        ...
        # 计算波动率
        daily_sorted['vol_2w'] = daily_sorted.groupby('ts_code')['ret_1w'].transform(
            lambda x: x.rolling(10).std()
        )
        
        # 计算换手率变化
        daily_sorted = daily_sorted.merge(
            self.basic[['ts_code', 'trade_date', 'turnover_rate']],
        ...
        for i, trade_date in enumerate(weekly_dates):
            ...
            date_data = date_data.merge(
                self.basic[['ts_code', 'trade_date', 'total_mv']],
                on=['ts_code', 'trade_date'],
                how='left'
            )
            ...
            if len(idx_data) > 0:
                index_trend = (idx_data['close'].values[0] - idx_data['ma_20w'].values[0]) / idx_data['ma_20w'].values[0] * 100
            else:
                index_trend = np.nan
    ...
    def save_results(self):
        ...
        logger.info("=" * 70)
        latest_data = self.regime_df.tail(10)[['trade_date', 'trend_raw', 'liquidity_raw', 'fundamental_raw', 'speculative_raw']]
