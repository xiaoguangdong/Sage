...
class ConceptActiveAnalyzer:
    ...
    def calculate_concept_activity(self, data):
        ...
        for concept_code in data['ts_code'].unique():
            ...
            # 8. 领涨次数（涨跌幅排名前10%）
            # 计算每日排名
            concept_data['daily_rank'] = concept_data.groupby('trade_date')['pct_change'].rank(ascending=False, pct=True)
    ...
    def classify_mainline(self, mainline_df):
        ...
        for idx, row in mainline_df.iterrows():
            ...
            # 根据关键词分类
            if any(keyword in name for keyword in ['芯片', '半导体', '集成电路', '电子', '通信', '5G', '6G', '光刻', '存储']):
                category = '科技主线'
    ...
    def print_summary(self, activity_df, mainline_df):
        ...
        for i, row in activity_df.head(20).iterrows():
            print(f"{i+1:<4} {row['name']:<20} {row['activity_score']:>11.1f} {row['cumulative_return']:>11.2f}% {row['up_ratio']*100:>11.1f}% {row['leading_ratio']*100:>11.1f}%")
        
        print(f"\n2. 主线逻辑Top 10:")
        print(f"{'排名':<4} {'概念名称':<20} {'主线分类':<12} {'上榜次数':<12} {'累计涨幅':<12} {'活跃度评分':<12}")
        print("-" * 100)
        for i, row in mainline_df.head(10).iterrows():
            print(f"{i+1:<4} {row['name']:<20} {row['mainline_category']:<12} {row['appearance_count']:>11} {row['cumulative_return']:>11.2f}% {row['activity_score']:>11.1f}")
