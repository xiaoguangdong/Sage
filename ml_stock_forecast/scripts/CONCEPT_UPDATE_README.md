...
# 初始化：获取基准数据（如2024年9月）
python ml_stock_forecast/scripts/update_concept_data.py --init

# 周度更新：获取最新数据并计算表现
python ml_stock_forecast/scripts/update_concept_data.py --update \
    --start-date 2024-09-24 \
    --end-date 2024-12-31 \
    --min-stock-count 10

# 只计算表现（使用已有的概念数据）
python ml_stock_forecast/scripts/update_concept_data.py --calculate \
    --start-date 2024-09-24 \
    --end-date 2024-12-31 \
    --min-stock-count 10
```
...
```bash
# 初始化
./ml_stock_forecast/scripts/update_concept_data.sh init

# 周度更新
./ml_stock_forecast/scripts/update_concept_data.sh update

# 只计算表现
./ml_stock_forecast/scripts/update_concept_data.sh calc
```
...
# 添加以下内容（每周五下午3点执行）
0 15 * * 5 cd /Users/dongxg/SourceCode/deep_final_kp && ./ml_stock_forecast/scripts/update_concept_data.sh update >> logs/concept_update_cron.log 2>&1
```
