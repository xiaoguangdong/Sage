...
class LabelingMethodComparison:
    ...
    def resample_weekly(self):
        """按周重采样"""
        df_weekly = self.df.resample('W', on='date').agg({
            'close': 'last',
            'open': 'first',
            'high': 'max',
            'low': 'min',
...
def main():
    ...
    methods = comparator.compare_methods()
    
    print("\n" + "=" * 80)
    print("对比完成！")
    print("=" * 80)
    print(f"图表: images/label/labeling_methods_comparison.png")
    print(f"数据: images/label/labeling_methods_comparison.csv")
