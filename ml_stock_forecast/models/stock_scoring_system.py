...
class TradingDecisionEngine:
    ...
    def make_decision(self, df: pd.DataFrame, stock_code: str = None, 
                     position_info: Dict = None, account_value: float = 1000000) -> Dict:
        """
        生成交易决策
        ...
        if position_info and position_info.get('shares', 0) > 0:
            decision = self._decide_for_holding(df, buy_signals, sell_signals, trend, 
                                               position_info, support_levels, resistance_levels)
        else:
            decision = self._decide_for_buying(df, buy_signals, sell_signals, trend, 
                                              score_result, account_value, support_levels, resistance_levels)
    ...
    def _detect_buy_signals(self, df: pd.DataFrame) -> List[Dict]:
        ...
        if (len(df) >= 3 and 
            ...
            df['ma5'].iloc[-2] <= df['ma20'].iloc[-2] and
        ...
        rsi = df['rsi14'].iloc[-1]
        if (len(df) >= 3 and
            rsi < 30 and df['rsi14'].iloc[-2] < 30 and rsi > df['rsi14'].iloc[-2]):
            signals.append({
                "type": "rsi_oversold",
                "confidence": 0.6
            })
        ...
        bb_position = df['bb_position'].iloc[-1]
        if (len(df) >= 3 and
            bb_position < 0.1 and df['close'].iloc[-1] > df['close'].iloc[-2]):
            signals.append({
        ...
        # 信号4: 上升突破
        if (len(df) >= 6 and
            df['close'].iloc[-1] > df['high'].iloc[-5:].max() and
    ...
    def _detect_sell_signals(self, df: pd.DataFrame) -> List[Dict]:
        ...
        # 信号3: 放量滞涨
        if (len(df) >= 3 and
            df['pctChg'].iloc[-1] < 0.01 and 
            df['volume_ratio'].iloc[-1] > 2.0):
            signals.append({
                "type": "volume_spike_no_gain",
                "confidence": 0.65
            })
        
        # 信号4: 下跌突破
        if (len(df) >= 6 and
            df['close'].iloc[-1] < df['low'].iloc[-5:].min() and
            df['volume_ratio'].iloc[-1] > 1.3):
            signals.append({
                "type": "breakdown",
                "confidence": 0.75
    ...
    def _decide_for_buying(self, df: pd.DataFrame, buy_signals: List[Dict], 
                          sell_signals: List[Dict], trend: str, 
                          score_result: Dict, account_value: float,
                          support_levels: Dict, resistance_levels: Dict) -> Dict:
        """
        空仓时的买入决策
    ...
    def _decide_for_holding(self, df: pd.DataFrame, buy_signals: List[Dict],
                           sell_signals: List[Dict], trend: str,
                           position_info: Dict, 
                           support_levels: Dict, resistance_levels: Dict) -> Dict:
        """
    ...
    def _calculate_take_profit(self, df: pd.DataFrame, stop_loss: float, 
                              resistance_levels: Dict) -> Dict:
        """
        计算止盈位
        ...
        return {
            ...
            "tp2": tp2,
            "tp3": tp3,
            "trailing_stop": tp1 * 0.95
        }
    
    def _get_missing_buy_conditions(self, buy_signals: List[Dict], 
                                   trend: str, score_result: Dict) -> List[str]:
        """获取缺失的买入条件"""
        missing = []
