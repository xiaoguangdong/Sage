...
- [ ] 与规则选股对照
- [ ] 与融合层闭环
- [ ] Champion/Challenger 治理落地（线上唯一冠军 + 影子挑战者）
- [ ] 策略命名与注册：`seed_balance_strategy` / `balance_strategy_v1` / `positive_strategy_v1` / `value_strategy_v1`
- [ ] `multi_alpha_selector.py` 拆分为三个 Challenger 并统一输出契约
- [ ] 执行层仅读取 `active_champion_id`，禁止多策略并行下单
