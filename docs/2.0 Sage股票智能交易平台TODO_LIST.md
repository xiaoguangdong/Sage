...
- [ ] **执行层解耦**：买卖点过滤/仓位控制/风控独立模块，仅消费「信号+行情+配置」
- [ ] **TWAP执行算法**：时间加权平均价格
- [ ] **VWAP执行算法**：成交量加权平均价格
- [ ] **智能拆单**：大单分批执行
- [ ] **盘口深度考虑**：流动性约束
...
- [ ] **数据完整性检查**：断点检测与补数
- [ ] **数据新鲜度监控**：最后更新时间
- [ ] **单向数据流转规范**：raw → processed → features → signals → backtest（禁止逆流）
- [ ] **数据访问层收敛**：核心模型禁止直接读 raw，读取逻辑统一到 `sage_app/data`
- [ ] **数据源扩展**
  - [ ] Eastmoney财报数据接入
  - [ ] Eastmoney事件数据接入
  - [ ] Akshare数据源接入评估
...
- [ ] **日志与监控**
  - [ ] 结构化日志（JSON格式）
  - [ ] 日志聚合（集中到单一目录结构）
  - [ ] 日志字段规范化：`任务名/窗口/耗时/失败原因` 全量输出
  - [ ] 日志命名强制校验：`logs/<module>/YYYYMMDD_*`
...
- [ ] **治理阈值配置化**：收益/回撤/换手/成本后收益（统一在 config）
- [ ] **晋升/降级日志**：明确输出原因、阈值、结果
- [ ] **自动降级机制**：连续N期表现不佳触发
- [ ] **切换审计日志**
- [ ] **A/B测试机制**
  - [ ] 建立策略A/B测试框架
