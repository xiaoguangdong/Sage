...
## 0. 现状清单（基于当前仓库）
### 已实现（雏形/基础）
- 宏观/北向/NBS 数据拉取脚本：`scripts/macro/*`
- 趋势/排序/买卖点模型雏形：`ml_stock_forecast/models/*`
- 主线与行业轮动原型脚本：`ml_stock_forecast/scripts/mainline_logic_system*.py`
- Walk-forward 回测框架（但收益计算仍为占位）
...
### 数据源适配器（细分）
- [x] Tushare/Baostock 接口存在（`data_provider.py`）
- [x] NBS/Tushare 宏观脚本存在（`scripts/macro/*`）
- [ ] Eastmoney 接入
- [ ] Akshare 接入
...
- Q: 是否保留/迁移 `ml_stock_forecast` 作为旧版基线？  
  A: 先在 `ml_stock_forecast` 上开发，后续再定命名与迁移。
- Q: 低频调仓目标是“周/双周/月”？  
  A: 周频调仓（已确认）。
