...
统一批量下载入口：`scripts/data/tushare_batch_download.sh`

```bash
# 下载全部任务（30个，按依赖顺序串行执行）
nohup bash scripts/data/tushare_batch_download.sh > logs/data/download_all.log 2>&1 &

# 只下载指定任务
bash scripts/data/tushare_batch_download.sh ths_daily ths_member ths_index

# 指定日期范围 + 指定任务
bash scripts/data/tushare_batch_download.sh --start 20200101 --end 20241231 daily_kline daily_basic

# 查看所有可用任务
bash scripts/data/tushare_batch_download.sh --list

# 调整请求间隔（默认40秒）
bash scripts/data/tushare_batch_download.sh --sleep 60 fina_mainbz_vip
```
...
- 补数计划：`config/download_plans.yaml`
...
产出文件：
- `data/signals/portfolio/portfolio_YYYYMMDD.csv` — 目标持仓与权重
- `data/signals/portfolio/execution_context_YYYYMMDD.json` — 决策上下文

长线选股基线模式：
- 当前默认 `rule_only: true`，仅使用规则托底（不启用模型）
- 基线期：2026-02-20 ~ 2026-03-20（4周）
- 开关与阈值在 `config/app/portfolio_manager.yaml` 的 `long_term_selector.rule_config` 中统一配置
- 当 `rule_only: false` 且 `rule_fallback: true` 时，候选不足会回退规则池
- 行业分位数过滤也在同一配置中调整：`industry_col`（默认申万L1）、`top_pct`（如0.30）、`min_samples`（如30）
- 基线期结束后需手动切换 `rule_only: false`
