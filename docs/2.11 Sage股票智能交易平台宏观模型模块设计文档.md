# Sage股票智能交易平台宏观模型模块设计文档

## 1. 模块定位
宏观模型用于**识别行业景气度与场景**，不直接给买卖点，只输出行业机会与风险等级，作为选股与信号融合的“方向盘”。

---

## 2. 输入 / 输出
**输入**：
- NBS 行业数据：PPI分项、FAI、产量等（月度/季度）
- Tushare 宏观：CPI/PPI/PMI/利率/社融等
- 北向资金/资金流（可选）
- 行业映射（NBS → 申万）

**输出**：
```json
{
  "date": "2026-02-13",
  "systemic_scenario": "NORMAL",
  "opportunity_industries": [
    {"industry": "电力设备", "scenario": "RECOVERY", "confidence": 0.72}
  ],
  "risk_level": "MEDIUM"
}
```

**输出落地（建议）**：
- `data/signals/macro_opportunity_<YYYYMM>.parquet`
- 字段：`date/industry/scenario/confidence/boom_score`

---

## 3. 核心逻辑
1. **系统性风险判定**
   - PMI、社融增速等触发 “SYSTEMIC RECESSION”
2. **行业景气度评分**
   - PPI改善、FAI增速、库存拐点、估值分位
3. **场景分类**
   - RECOVERY / BOOM / RECESSION / NEUTRAL
4. **置信度评分**
   - 指标偏离阈值程度 + 一致性
5. **资金流信号（市场级）**
   - 北向资金净流入布林带突破（市场级，不做行业配置比例）

### 3.1 指标清单（MVP v1）
- 宏观：`pmi`, `credit_growth`, `yield_10y`, `cpi_yoy`, `ppi_yoy`
- 行业：`ppi_industry_yoy`, `fai_yoy`, `output_yoy`, `rev_yoy`, `pb_percentile`
- 资金（市场级）：`northbound_net_flow`（布林带突破）

### 3.2 流程图（Mermaid）
```mermaid
flowchart LR
  A["宏观/NBS/资金数据"] --> B["行业映射与对齐"]
  B --> C["指标计算/评分"]
  C --> D["场景分类+置信度"]
  D --> E["行业机会列表"]
```

---

## 4. 数据延迟
财报/公告/北向等按 **T+2** 延迟，严格防止前视。

---

## 5. 与现有代码映射
- `ml_stock_forecast/models/macro_predictor.py`
- `ml_stock_forecast/models/industry_classifier.py`
- `ml_stock_forecast/models/cycle_detector.py`

---

## 6. 测试策略
- 单元测试：场景分类边界  
- 回测验证：是否提升行业轮动与回撤  

---

## Q&A
- Q: 行业映射口径是否统一申万一级？  
  A: 当前默认申万一级，后续可配置。
