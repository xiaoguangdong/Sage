...
- `concept_heat_score`：综合活跃度评分（已实现，来源 ths_daily）

**概念→行业映射（辅助信号）**：
- 输出：`data/processed/concepts/concept_industry_coverage.parquet`
- 输出：`data/processed/concepts/concept_industry_primary.parquet`
- 用途：为行业主线/轮动提供“概念偏置”或风险提示（不直接交易）
