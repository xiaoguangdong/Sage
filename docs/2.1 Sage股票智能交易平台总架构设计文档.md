...
## 4.1 周度流程图（run_weekly.py 细化）
```mermaid
flowchart TB
  W0["周度触发<br/>scripts/run_job.py weekly_pipeline"] --> W1["加载配置<br/>config/app/*.yaml"]
  W1 --> W2["加载数据<br/>DataLoader + data/tushare"]
  W2 --> W3["特征计算<br/>Price/Market/LongTerm"]
  W3 --> W4["行业映射补全<br/>industry_l1 对齐"]
  W4 --> W5["趋势模型<br/>HS300 状态 + 置信度"]
  W3 --> W6["选股/排序<br/>Rule/LGBM/XGB/Seed"]
  W6 --> W7["策略治理<br/>Champion/Challenger"]
  W5 --> W7
  W3 --> W8["长周期选股<br/>Value/Growth Selector"]
  W2 --> W9["政策/概念/北向信号<br/>industry_signal_contract"]
  W7 --> W10["统一信号契约<br/>stock_signal_contract"]
  W8 --> W10
  W9 --> W11["行业叠加 Overlay"]
  W10 --> W11
  W11 --> W12["组合构建<br/>PortfolioManager/Construction"]
  W12 --> W13["EntryModel 过滤"]
  W5 --> W14["仓位管理<br/>confidence/趋势门控"]
  W13 --> W15["增强风控<br/>ATR/分档降仓/行业止损"]
  W14 --> W15
  W15 --> W16["输出组合<br/>portfolio_YYYYMMDD.csv"]
  W16 --> W17["执行上下文<br/>execution_context_YYYYMMDD.json"]
  W16 --> W18["归因快照<br/>factor_exposure + industry_weight"]
  W18 --> W19["收益回填<br/>fill_attribution_returns"]
  W19 --> W20["归因报告<br/>Brinson/Factor"]
```

---

## 4.1.1 日度流程图（trend/stock scheduler）
```mermaid
flowchart TB
  D0["日度触发<br/>scripts/run_job.py stock_scheduler / trend_scheduler"] --> D1["加载配置<br/>config/app/*.yaml"]
  D1 --> D2["数据更新<br/>tushare_downloader + 宏观/政策抓取"]
  D2 --> D3["数据治理<br/>清洗/对齐/T+2 延迟"]
  D3 --> D4["趋势信号更新<br/>trend_signal_scheduler"]
  D3 --> D5["选股信号更新<br/>stock_selector_scheduler"]
  D4 --> D6["趋势输出<br/>trend_signals_YYYYMMDD.csv"]
  D5 --> D7["选股信号输出<br/>stock_signal_contract_YYYYMMDD.parquet"]
  D6 --> D8["行业叠加信号更新<br/>industry_signal_contract"]
  D7 --> D8
  D8 --> D9["日度风控止损检查（可选）<br/>ATR/行业/冲击"]
  D9 --> D10["监控与巡检<br/>data_integrity/quality checks"]
```
...
| D7 日度风控（可选） | 持仓止损/降仓检查（ATR/行业/冲击） | `执行层调用（非独立脚本）` | `stop_loss_report` / 风控日志 | 交易执行/告警 |
| D8 巡检监控 | 数据完整性与异常检查 | `scripts/macro/check_macro_data.sh`（及后续巡检） | 检查日志/告警 | 运维与回补 |
...
**ATR 止损通俗解释**：把 ATR 当作“波动通道的宽度”。用近 `atr_period` 天的真实波动幅度（TR）求平均得到 ATR，然后用 `entry_price - ATR×倍数` 作为止损线；价格跌破这条线就卖出。如果历史不足或缺高低价，则退化为固定 `-8%` 止损。
