...
ml_stock_forecast/   # 非核心管线（数据接入/调度/入口）
  non_core/
    config/
    data/
    pipelines/

sage/             # 平台骨架（注册器/配置/类型定义）
scripts/          # 分模块脚本（data/models/strategy/backtest/monitoring/legacy）
logs/             # 分模块日志
```

### 2.3 现有代码映射
- `ml_stock_forecast/*` 核心模块 → `sage_core/*`
- `ml_stock_forecast/run_weekly.py` → `ml_stock_forecast/non_core/pipelines/run_weekly.py`
- `scripts/macro/*` → `scripts/data/macro/`
- `mainline_logic_system*.py` → `scripts/strategy/`
