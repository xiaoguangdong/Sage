...
**P0: 极端行情熔断（优先级最高）**
- 单日跌幅 ≥ 3.5% → 强制 RISK_OFF
- 连续3日累计跌幅 ≥ 6% → 强制 RISK_OFF
- 连续5日累计跌幅 ≥ 8% → 强制 RISK_OFF
- 从20日滚动高点回撤 ≥ 8% → 强制 RISK_OFF
- 熔断触发后强制保持 RISK_OFF 5天
- 阈值针对A股特点调优（A股极端行情多为连续2-3%/天的阴跌，而非单日暴跌）

**P1: 波动率自适应退出宽容期**
- 高波动时缩短宽容期（快速退出），低波动时延长宽容期（避免假退出）
- `adaptive_tolerance = exit_tolerance × clip(vol_median / vol_current, 0.5, 1.5)`
- vol_current: 20日滚动波动率；vol_median: 120日波动率中位数
- 最小宽容期为3天

**P2: 多时间框架确认**
- 入场时需要周线级别均线（MA65 ≈ 13周×5）同向确认
- 牛市入场：日线 bull 条件 + close > MA_weekly
- 熊市入场：日线 bear 条件 + close < MA_weekly
- 退出不受周线约束（仅影响入场）
...
```mermaid
flowchart TB
  A["指数OHLCV"] --> B["计算MA/slope/vol/drawdown"]
  B --> CB{"P0: 熔断检查<br/>单日/3日/5日跌幅<br/>20日高点回撤"}
  CB -->|触发| R0["强制RISK_OFF<br/>保持5天"]
  CB -->|未触发| C["硬条件判断<br/>bull/bear"]
  C --> P1["P1: 波动率自适应<br/>调整退出宽容期"]
  P1 --> P2["P2: 多时间框架<br/>周线确认入场"]
  P2 --> D["状态机<br/>入场确认 + 退出宽容期<br/>+ 最小持续期"]
...
| **P0: 极端行情熔断** | | |
| circuit_breaker_1d | -0.035 | 单日跌幅阈值 |
| circuit_breaker_3d | -0.06 | 3日累计跌幅阈值 |
| circuit_breaker_5d | -0.08 | 5日累计跌幅阈值 |
| circuit_breaker_drawdown | -0.08 | 20日高点回撤阈值 |
| circuit_breaker_hold | 5 | 熔断后强制RISK_OFF天数 |
| **P1: 波动率自适应** | | |
| vol_adaptive | true | 是否启用波动率自适应 |
| vol_window | 20 | 当前波动率窗口 |
| vol_median_window | 120 | 中位波动率窗口 |
| **P2: 多时间框架** | | |
| mtf_enabled | true | 是否启用多时间框架确认 |
| ma_weekly | 13 | 周线级别均线（×5=65日） |
...
rule_params:
  ...
  # P0: 极端行情熔断
  circuit_breaker_1d: -0.035
  circuit_breaker_3d: -0.06
  circuit_breaker_5d: -0.08
  circuit_breaker_drawdown: -0.08
  circuit_breaker_hold: 5
...
### Q7: 如何处理极端行情（如熔断）？
**A**（已实现 P0 熔断机制）:
- 单日跌幅 ≥ 3.5%、3日 ≥ 6%、5日 ≥ 8%、20日高点回撤 ≥ 8% → 强制 RISK_OFF
- 熔断后强制保持 5 天，优先级高于一切状态机逻辑
- 阈值针对A股特点调优（2021年2月急跌验证通过）
- 配合组合层面的单日冲击止损
