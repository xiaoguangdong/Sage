...
class EntryModelLR:
    ...
    def __init__(self, config: dict):
        ...
        # 模型参数
        self.model_params = {"max_iter": 1000, "random_state": 42, "class_weight": "balanced"}

        # 规则参数
        self.rules = {"prob_threshold": 0.6, "ma_period": 20, "vol_threshold": 0.05}
        ...
        # 标签参数
        self.label_params = {"horizon": 10, "return_threshold": 0.05, "max_drawdown": -0.03}
    ...
    def _prepare_features_single(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in [5, 10, 20, 60]:
            ...
            df[f"price_std{period}_ratio"] = (df["close"] - df["close"].rolling(period).mean()) / df["close"].rolling(
                period
            ).std()
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "label": {"horizon": 10, "return_threshold": 0.05, "max_drawdown": -0.03},
    }
