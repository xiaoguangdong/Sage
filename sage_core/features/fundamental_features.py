...
class FundamentalFeatures(FeatureGenerator):
    ...
    def calculate_growth_features(
        ...
        if fina_indicator is not None and not fina_indicator.empty:
            ...
            # 合并成长相关字段
            growth_cols = ['ts_code', 'date', 'or_yoy', 'netprofit_yoy', 'dt_netprofit_yoy',
                          'profit_dedt', 'oper_rev', 'ebt_yoy', 'tr_yoy']
        ...
        if growth_components:
            ...
            df['growth_score'] = sum(
                df.get(c, 0) * growth_weights.get(c, 1/len(growth_components))
                for c in growth_components
            )
        else:
    ...
    def calculate_all_features(
        ...
        new_features = [c for c in df.columns if c not in base_cols or
                        c in ['pe_ttm', 'pb', 'pe_percentile', 'pb_percentile',
                              ...
                              'revenue_yoy', 'profit_yoy', 'peg', 'growth_score',
                              'cashflow_to_profit', 'fundamental_score']]
    ...
    def get_feature_names(self) -> List[str]:
        ...
        return [
            ...
            # 质量因子（30%）
            'roe', 'gross_margin', 'roic', 'debt_to_assets',
            'roe_change', 'gross_margin_change', 'roic_change',
            'roe_ma4', 'gross_margin_ma4', 'roic_ma4',
            'roe_zscore', 'gross_margin_zscore', 'roic_zscore',
            'cashflow_to_profit', 'cashflow_quality_zscore',
            'quality_score',
            # 成长因子（30%）
            'revenue_yoy', 'profit_yoy',
            'revenue_yoy_accel', 'profit_yoy_accel',
            'revenue_yoy_zscore', 'profit_yoy_zscore', 'profit_yoy_accel_zscore',
            'peg',
            'growth_score',
            # 综合评分
            'fundamental_score',
