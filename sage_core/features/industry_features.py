...
class IndustryFeatures(FeatureGenerator):
    ...
    def _merge_industry_features(
        ...
        if "industry_code" in stock_df.columns:
            # 个股数据已有行业代码，直接合并
            merge_df = stock_df.merge(industry_df, on=["date", "industry_code"], how="left", suffixes=("", "_ind"))
            ...
            if "industry_name" in industry_df.columns:
                ...
                merge_df = stock_df.merge(industry_df, on=["date", "industry_name"], how="left", suffixes=("", "_ind"))
            else:
                # 按行业分组计算平均动量
                # 先计算每个日期每个行业的平均动量
                (
                    industry_df.groupby(["date", "industry_code"])
                    .agg({f"industry_ret_{w}w": "first" for w in self.momentum_windows})
                    .reset_index()
                )
    ...
    def calculate_industry_relative_features(
        ...
        if "pe_ttm" in df.columns:
            df["industry_pe_rank"] = df.groupby([industry_col, "date"])["pe_ttm"].transform(lambda x: x.rank(pct=True))
