...
class PriceFeatures(FeatureGenerator):
    ...
    def calculate_momentum_features(
        ...
        if "amount" in df.columns:
            # 个股成交额在全市场的分位数
            df["amt_quantile"] = df.groupby("date")["amount"].transform(lambda x: x.rank(pct=True))
        ...
        if mom_components:
            ...
            df["momentum_score"] = sum(
                df.get(c, 0) * momentum_weights.get(c, 1 / len(mom_components)) for c in mom_components
            )
        else:
    ...
    def calculate_liquidity_features(self, df: pd.DataFrame, periods: List[int] = [4, 12]) -> pd.DataFrame:
        """
        计算流动性特征
        ...
        for period in periods:
            ...
            if volume_group is not None and float_group is not None:
                df[f"turnover_{period}w"] = volume_group.transform(
                    lambda s: s.rolling(period).mean()
                ) / float_group.transform(lambda s: s.rolling(period).mean())
        ...
        if volume_group is not None:
            df["volume_trend"] = volume_group.transform(lambda s: s.rolling(5).mean()) / volume_group.transform(
    ...
    def calculate_stability_features(self, df: pd.DataFrame, periods: List[int] = None) -> pd.DataFrame:
        """
    ...
    def get_feature_names(self) -> List[str]:
        ...
        return [
            ...
            "excess_return_vs_industry_60d",
            "excess_return_vs_industry_20d_zscore",
            "excess_return_vs_industry_60d_zscore",
            "amt_quantile",
            "amt_ratio_20_250",
            "amt_quantile_zscore",
            "momentum_score",
