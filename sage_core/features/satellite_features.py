...
class SatelliteFeatures(FeatureGenerator):
    ...
    def calculate_growth_factor(
        ...
        if fina_indicator is not None and not fina_indicator.empty:
            ...
            # 取每只股票最近2期财报
            fina_cols = ['ts_code', 'date', 'tr_yoy', 'or_yoy', 'netprofit_yoy',
                         'roe_ttm', 'grossprofit_margin', 'netprofit_margin',
                         'net_cash_operate_act', 'net_profit']
        ...
        if industry_rank is not None and not industry_rank.empty:
            if 'industry' in df.columns and 'industry_name' in industry_rank.columns:
                ...
                df['industry_score'] = df['industry_rank'].apply(
                    lambda x: 5 if pd.notna(x) and x <= 0.3 else 0
                )
            else:
                df['industry_score'] = 0
        else:
        ...
        # 景气成长因子总分
        df['growth_factor_score'] = (
            df['profit_growth_score'] +
    ...
    def calculate_event_factor(
        ...
        if rating_data is not None and not rating_data.empty:
            # 机构评级上调/首次覆盖
            rating_up_codes = rating_data[
                rating_data.get('rating_change', '').isin(['上调', '首次'])
        ...
        if survey_data is not None and not survey_data.empty:
            # 密集调研（近1个月调研机构≥10家）
            survey_codes = survey_data[
                survey_data.get('org_count', 0) >= 10
            ]['ts_code'].unique().tolist()
    ...
    def calculate_momentum_factor(
        ...
        df['ma50'] = df.groupby('ts_code')['close'].transform(
            lambda x: x.rolling(50, min_periods=25).mean()
        )

        # 计算成交量均值
        if 'volume' in df.columns:
            df['vol_20d'] = df.groupby('ts_code')['volume'].transform(
                lambda x: x.rolling(20, min_periods=10).mean()
            )
            df['vol_60d'] = df.groupby('ts_code')['volume'].transform(
    ...
    def calculate_chip_factor(
        ...
        if top_list_data is not None and not top_list_data.empty:
            # 机构专用席位净买入为正
            org_buy = top_list_data[
                top_list_data.get('exalter', '').str.contains('机构专用', na=False)
            ].groupby('ts_code')['net_buy'].sum()
        ...
        if northbound_data is not None and not northbound_data.empty:
            # 北向资金持续增持
            northbound_increase_codes = northbound_data[
                northbound_data.get('hold_ratio_change', 0) > 0
            ]['ts_code'].unique().tolist()
    ...
    def calculate_satellite_score(
        ...
        df['satellite_score'] = (
            df['growth_factor_score'] +
            df['event_factor_score'] +
            df['momentum_factor_score'] +
            df['chip_factor_score']
        )
    ...
    def get_feature_names(self) -> List[str]:
        ...
        return [
            # 景气成长因子（40分）
            'profit_growth_score', 'profitability_score', 'industry_score',
            'growth_factor_score',
            # 事件驱动因子（30分）
            'has_core_event', 'has_rating_up', 'has_survey', 'event_certainty',
            'event_factor_score',
            # 量价动量因子（20分）
            'trend_score', 'momentum_score', 'volume_score',
            'momentum_factor_score',
            # 筹码结构因子（10分）
            'holder_decreasing', 'org_net_buy', 'northbound_increase',
            'chip_factor_score',
            # 综合评分
            'satellite_score', 'in_select_pool', 'stock_type',
            # 仓位限额
            'init_position', 'max_position', 'position_ratio',
        ]
