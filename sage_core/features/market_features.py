...
class MarketFeatures(FeatureGenerator):
    ...
    def calculate_price_rank_features(self, df: pd.DataFrame, windows: List[int] = [20, 60]) -> pd.DataFrame:
        """
        计算价格排名特征
        ...
        for window in windows:
            df[f"price_rank_{window}d"] = (
                df["close"].rolling(window).apply(lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=False)
            )
    ...
    def calculate_price_volume_corr(self, df: pd.DataFrame, period: int = 4) -> pd.DataFrame:
        ...
        if "amount" in df.columns:
            df["price_volume_corr"] = (
                df["close"].pct_change(period).rolling(period).corr(df["amount"].pct_change(period))
            )
        else:
