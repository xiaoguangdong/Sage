    ...
    def calculate_northbound_features(
        ...
        if hk_hold is not None and not hk_hold.empty:
            ...
            if len(hold_cols) >= 3:
                ...
                hk_subset = hk_subset.rename(columns={
                    'hold_ratio': 'northbound_hold_ratio',
                    'hold_amount': 'northbound_hold_amount'
                })
        ...
        if 'northbound_hold_ratio' in df.columns:
            ...
            for window in self.northbound_windows:
                ...
                # 持股比例变化
                df[f'northbound_hold_ratio_{window}d_change'] = df.groupby('ts_code')['northbound_hold_ratio'].transform(
