...
class TrendModelEnhanced:
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 乖离率
        df['bias'] = (df['close'] - df['close'].rolling(self.ma_medium).mean()) / df['close'].rolling(self.ma_medium).mean() * 100
    ...
    def _detect_bull_state(self, df: pd.DataFrame, idx: int) -> Tuple[str, float]:
        ...
        # 2. 检测晚期牛市
        if (rsi > 75 and 
            volatility > vol_ma * 1.5 and
            bb_position > 0.8 and
            macd > 0 and ma20_slope > 0):
            return 'LATE_BULL', 0.75
        
        # 3. 检测确认牛市
        if (ma20_slope > 0 and ma60_slope > 0 and
            rsi > 50 and rsi < 70 and
            momentum_4w > 0 and
            volatility < vol_ma * 1.2):
            return 'CONFIRMED_BULL', 0.8
        
        # 4. 检测初始牛市
        if (ma20_slope > 0 and momentum_4w > 0 and
    ...
    def _detect_consolidation_state(self, df: pd.DataFrame, idx: int) -> Tuple[str, float]:
        ...
        # 2. 检测箱体震荡
        if (abs(ma20_slope) < 0.01 and abs(ma60_slope) < 0.01 and
            bb_width < 0.10 and
            rsi > 40 and rsi < 60):
        ...
        # 3. 检测上升中继
        if (ma20_slope > 0 and ma60_slope > 0 and
            momentum_4w > -0.02 and momentum_4w < 0.02 and
            bb_width < 0.08):
    ...
    def _detect_special_state(self, df: pd.DataFrame, idx: int) -> Optional[Tuple[str, float]]:
        ...
        if idx >= 5:
            ...
            if (recent_prices[-1] > recent_prices[0] and 
                recent_macd[-1] < recent_macd[0] and 
                macd > 0):
