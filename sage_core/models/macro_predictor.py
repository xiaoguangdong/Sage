...
class MacroPredictor:
    ...
    def predict(
        ...
        for sw_industry in sw_industries:
            ...
            industry_metrics = self._get_industry_metrics(
                sw_industry, date, industry_data, macro_data, northbound_data
            )
    ...
    def _classify_scenario(self, metrics: Dict) -> str:
        ...
        """
        # 1. 判断行业大涨
        if (metrics['pb_percentile'] > self.thresholds['boom']['pb_percentile'] and
            metrics['turnover_rate'] > self.thresholds['boom']['turnover_rate'] and
            metrics['rps_120'] > self.thresholds['boom']['rps_120']):
            return "BOOM / BUBBLE"
        ...
        # 3. 判断行业衰退
        if (metrics['ppi_yoy'] < self.thresholds['recession']['ppi_yoy'] and
            (metrics['fai_yoy'] is None or metrics['fai_yoy'] < self.thresholds['recession']['fai_yoy']) and
            (metrics['rev_yoy'] is None or metrics['rev_yoy'] < self.thresholds['recession']['rev_yoy'])):
