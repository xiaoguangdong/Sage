...
from sage_core.features import (
    PriceFeatures,
    FundamentalFeatures,
    FlowFeatures,
    IndustryFeatures,
...
class RankModelLGBM:
    ...
    def prepare_features(
        ...
        if daily_basic is not None or fina_indicator is not None:
            df = self.fundamental_features.calculate_all_features(
                df, daily_basic, fina_indicator
            )

        # 3. 资金流向特征
        if hk_hold is not None or moneyflow is not None:
            df = self.flow_features.calculate_all_features(
                df, hk_hold=hk_hold, moneyflow=moneyflow
            )
    ...
    def _select_features_by_budget(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        feature_categories = {
            'quality': [
                'roe', 'gross_margin', 'roic', 'quality_score',
                'roe_zscore', 'gross_margin_zscore', 'roic_zscore',
            ],
            'momentum': [
                'return_4d', 'return_12d', 'return_20d',
                'ma_ratio_4d', 'ma_ratio_12d', 'ma_ratio_20d',
                'rsi_14', 'macd', 'macd_signal',
            ],
            'flow': [
                'northbound_hold_ratio', 'northbound_score',
                'main_net_inflow', 'main_flow_score',
                'northbound_net_flow_5d_sum',
            ],
            'risk': [
                'volatility_4d', 'volatility_12d',
                'price_std_4d', 'price_std_12d',
            ...
            'industry': [
                'industry_ret_4w', 'industry_ret_12w',
    ...
    def train(
        ...
        exclude_cols = {'date', 'trade_date', 'code', 'ts_code', 'stock', 'close',
                        'turnover', 'open', 'high', 'low', 'volume', 'amount',
                        'industry_l1', 'industry_l2', 'industry_name', 'industry_code'}
