...
def _evaluate_window(
    ...
    panel_sorted["future_ret"] = (
        panel_sorted.groupby("ts_code")["close"].pct_change(label_horizon).shift(-label_horizon)
    )

    eval_df = predictions.merge(
        panel_sorted[["ts_code", "trade_date", "future_ret"]], on=["ts_code", "trade_date"], how="left"
    )
    ...
    rank_ic = (
        ...
        .apply(lambda g: g["score"].rank(pct=True).corr(g["future_ret"].rank(pct=True)))
        .mean()
    )
...
def main() -> None:
    ...
    if results:
        ...
        # 保存最新
        (output_root / "walk_forward_results_latest.parquet").write_bytes(df_results.to_parquet(index=False))
