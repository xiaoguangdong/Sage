...
def load_realistic_data():
    ...
    # 转换为数值类型
    northbound_flow["ggt_ss"] = pd.to_numeric(
        northbound_flow["ggt_ss"], errors="coerce"
    )  # 沪股通（上交所北向累计持仓市值）
    northbound_flow["ggt_sz"] = pd.to_numeric(
        northbound_flow["ggt_sz"], errors="coerce"
    )  # 深股通（深交所北向累计持仓市值）
    ...
    print("  ⚠️  注意: 北向资金数据说明:")
    print("     - ggt_ss 和 ggt_sz 是累计持仓市值（亿元）")
    print("     - 通过差分计算日度净流入额（net_flow）")
    print(
        f"     - 2024年Q4平均日度净流入: {northbound_flow[northbound_flow['trade_date'].between('2024-09-01', '2024-12-31')]['net_flow'].mean():.2f}亿元"
    )
    ...
    print("  ⚠️  说明: PPI和FAI数据来自NBS真实数据")
    print("  ⚠️  估值和流动性数据使用模拟（需要从股票市场获取）")
...
def load_nbs_industry_data(start_date="2020-01-01", end_date="2026-12-31"):
    ...
    for sw_industry in sw_industries:
        ...
        for date in dates:
            ...
            industry_ppi.append({"sw_industry": sw_industry, "date": date, "ppi_mom": ppi_value})
    ...
    for sw_industry in sw_industries:
        ...
        for date in dates:
            ...
            industry_fai.append({"sw_industry": sw_industry, "date": date, "fai_yoy": fai_value})
    ...
    for sw_industry in sw_industries:
        ...
        for i in range(12, len(sw_data)):
            sw_data.iloc[i, sw_data.columns.get_loc("ppi_yoy")] = (
                sw_data.iloc[i]["cumulative_ppi"] / sw_data.iloc[i - 12]["cumulative_ppi"] - 1
            ) * 100

        # 更新数据
        industry_ppi_df.loc[industry_ppi_df["sw_industry"] == sw_industry, "cumulative_ppi"] = sw_data[
            "cumulative_ppi"
        ].values
        industry_ppi_df.loc[industry_ppi_df["sw_industry"] == sw_industry, "ppi_yoy"] = sw_data["ppi_yoy"].values

    # 7. 返回结果
    result = {"industry_ppi": industry_ppi_df, "industry_fai": industry_fai_df, "sw_industries": sw_industries}
...
def run_backtest():
    ...
    if recession_days > 0:
        ...
        discoveries.append("✅ 无系统风险信号，市场环境相对稳定")
    ...
    discoveries.append(
        f"🌡️  2024年Q4宏观环境: PMI平均{avg_pmi:.1f}, PPI平均{avg_ppi:.2f}%, CPI平均{avg_cpi:.2f}%, 10Y国债{avg_yield:.2f}%"
    )
    ...
    # 保存结果
    output_file = build_output_path("backtest_2024_q4_realistic_results.csv")
