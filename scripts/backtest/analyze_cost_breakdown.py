...
def analyze_cost_breakdown():
    ...
    for case in test_cases:
        ...
        print("\n  买入成本:")
        print(
            f"    固定成本: {buy_cost['fixed_cost']:>10,.2f} 元 ({buy_cost['fixed_cost']/case['trade_value']*100:>6.4f}%)"
        )
        print(
            f"    滑点成本: {buy_cost['slippage_cost']:>10,.2f} 元 ({buy_cost['slippage_cost']/case['trade_value']*100:>6.4f}%)"
        )
        print(
            f"    市场冲击: {buy_cost['market_impact_cost']:>10,.2f} 元 ({buy_cost['market_impact_cost']/case['trade_value']*100:>6.4f}%)"
        )
        ...
        print("\n  卖出成本:")
        print(
            f"    固定成本: {sell_cost['fixed_cost']:>10,.2f} 元 ({sell_cost['fixed_cost']/case['trade_value']*100:>6.4f}%)"
        )
        print(
            f"    滑点成本: {sell_cost['slippage_cost']:>10,.2f} 元 ({sell_cost['slippage_cost']/case['trade_value']*100:>6.4f}%)"
        )
        print(
            f"    市场冲击: {sell_cost['market_impact_cost']:>10,.2f} 元 ({sell_cost['market_impact_cost']/case['trade_value']*100:>6.4f}%)"
        )
    ...
    for turnover in turnovers:
        ...
        print(
            f"{turnover:>6.1f} | {trade_value:>10,.0f} | {cost_rate:>6.4%} | {cost_value:>10,.2f} | {annual_cost:>14.2%}"
        )
