...
def extract_features(stock_data, top_k=30):
    ...
    for stock_code, df in stock_data.items():
        ...
        feature_cols = [
            ...
            if col not in ["date", "code", "open", "high", "low", "close", "volume", "amount", "label", "future_return"]
        ]
...
def run_backtest(model, stock_features):
    ...
    for stock_code, df in stock_features.items():
        ...
        # 准备数据
        feature_cols = [col for col in df.columns if col not in ["date", "code", "label"]][:30]
        ...
        for i in range(seq_length, len(X)):
            ...
            predictions.append({"stock": stock_code, "date": df.iloc[i]["date"], "prediction": pred, "actual": y[i]})
    ...
    if len(pred_df) > 0:
        ...
        logger.info("\n混淆矩阵:")
        cm = pd.crosstab(pred_df["actual"], pred_df["prediction"], rownames=["Actual"], colnames=["Predicted"])
