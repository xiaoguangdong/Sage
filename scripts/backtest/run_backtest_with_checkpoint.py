    ...
    for stock_code, df in stock_data.items():
        ...
        feature_cols = [col for col in df.columns
                       if col not in ['date', 'code', 'open', 'high', 'low', 'close',
                                     'volume', 'amount', 'future_return']][:input_dim]
...
def run_backtest(checkpoint, stock_features, input_dim, hidden_dim):
    ...
    for stock_code, df in stock_features.items():
        ...
        for i in range(seq_length, len(X)):
            ...
            with torch.no_grad():
                output = checkpoint['model_state_dict']
                # 这里简化处理，实际需要构建完整模型
                # 暂时跳过，直接统计准确率
                pass
...
def main():
    ...
    print(f"\n建议:")
    print(f"  1. 使用 main.py 重新训练（推荐）")
    print(f"  2. 修改代码以匹配checkpoint的完整模型架构")
