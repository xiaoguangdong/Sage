...
class WeeklyLabeler:
    ...
    def load_data(self):
        ...
        print(f"✓ 指数数据加载完成")
        print(f"  时间范围: {self.df_index['date'].min().strftime('%Y-%m-%d')} 至 {self.df_index['date'].max().strftime('%Y-%m-%d')}")
    ...
    def load_stock_data(self):
        ...
        # 筛选沪深300成分股
        self.df_daily_basic = self.df_daily_basic[
            self.df_daily_basic['ts_code'].isin(self.stock_list)
        ]
    ...
    def resample_to_weekly(self):
        ...
        # 指数数据重采样
        weekly_index = self.df_index.resample('W-MON', on='date').agg({
            'open': 'first',
            'high': 'max',
            'low': 'min',
            'close': 'last',
        ...
        print(f"✓ 指数周线数据生成完成")
        print(f"  时间范围: {weekly_index['date'].min().strftime('%Y-%m-%d')} 至 {weekly_index['date'].max().strftime('%Y-%m-%d')}")
    ...
    def calculate_weekly_indicators(self):
        ...
        print("✓ 周线指标计算完成")
        print(f"  MA20w, MA60w, MA_diff")
        print(f"  index_ret_4w, index_vol_4w")
    ...
    def calculate_breadth_and_new_high(self):
        ...
        # 按周聚合成分股数据
        weekly_stocks = self.df_daily_basic.resample('W-MON', on='date').agg({
            'pct_chg': lambda x: np.mean(x),  # 平均涨跌幅
            'ts_code': 'count'  # 股票数量
        }).reset_index()
        ...
        # 计算上涨股票占比
        # 需要原始数据来计算占比
        weekly_rising = self.df_daily_basic.groupby(
        ...
        # 这里简化处理：使用涨跌幅作为参考
        # 如果涨跌幅 > 5%，认为可能创新高
        weekly_new_high = self.df_daily_basic.groupby(
            pd.Grouper(key='date', freq='W-MON')
        ).apply(
        ...
        print("✓ 市场广度指标计算完成")
        print(f"  breadth: 上涨股票占比")
        print(f"  new_high_ratio: 创新高比例")
    ...
    def visualize(self):
        ...
        # 2. 市场状态
        ax2 = axes[1]
        colors = ['red' if l == 0 else 'white' if l == 1 else 'green' for l in df['label']]
        ...
        # 左轴：4周收益
        line1 = ax4.plot(df['date'], df['index_ret_4w'], label='4周收益', 
        ...
        # 右轴：上涨占比
        line2 = ax4_twin.plot(df['date'], df['breadth'], label='上涨占比', 
    ...
    def generate_summary(self):
        ...
        yearly_stats.columns = ['牛市周数', '震荡周数', '熊市周数',
                               '评分均值', '评分标准差', '评分最小', '评分最大',
                               '4周收益均值', '4周收益标准差',
                               '上涨占比均值', '上涨占比标准差',
                               'MA差均值', 'MA差标准差']
