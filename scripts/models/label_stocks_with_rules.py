...
class StockLabeler:
    ...
    def __init__(self, data_dir=None, output_dir="images/label", num_stocks=30, start_year=2020, end_year=2025):
        """
        初始化打标器
    ...
    def select_random_stocks(self):
        ...
        selected_stocks = random.sample(list(all_stocks), min(self.num_stocks, len(all_stocks)))
    ...
    def calculate_total_score(self, df):
        ...
        # 布林带位置（10%）
        bb_pos = (
            (df["close"].iloc[-1] - df["bb_lower"].iloc[-1]) / (df["bb_upper"].iloc[-1] - df["bb_lower"].iloc[-1])
            if (df["bb_upper"].iloc[-1] - df["bb_lower"].iloc[-1]) > 0
            else 0.5
        )
    ...
    def plot_stock(self, stock_code, stock_name, df, labels_3state, labels_12state):
        ...
        scores = [self.calculate_total_score(df.iloc[: i + 1]) if i >= 250 else 0 for i in range(len(df))]
    ...
    def process_stocks(self):
        ...
        for i, stock_code in enumerate(selected_stocks, 1):
            ...
            try:
                # 获取股票数据
                stock_data = self.filtered_data[self.filtered_data["ts_code"] == stock_code].copy()
                ...
                # 绘图
                output_path = self.plot_stock(stock_code, stock_name, weekly_data, labels_3state, labels_12state)
