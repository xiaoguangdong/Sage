...
class HS300Labeler:
    ...
    def plot_labels(self, df, labels_3state, labels_12state, state_names):
        ...
        for i in range(len(df)):
            ...
            ax1.plot([x_values[i], x_values[i]], [low_price, high_price], color=color, linewidth=0.5)
            ax1.plot([x_values[i] - 0.3, x_values[i] + 0.3], [open_price, open_price], color=color, linewidth=1)
            ax1.plot([x_values[i] - 0.3, x_values[i] + 0.3], [close_price, close_price], color=color, linewidth=1)
        ...
        for ax in [ax1, ax2, ax3, ax4, ax5]:
            ...
            ax.set_xticklabels(dates.dt.strftime("%Y-%m").iloc[:: max(1, len(df) // 10)], rotation=45, fontsize=8)
    ...
    def process(self):
        ...
        # 计算Total_Score序列
        total_scores = [self.calculate_total_score(i) if i >= 250 else 0 for i in range(len(self.df))]
        ...
        output_plot = weekly_labeler.plot_labels(df_weekly, weekly_labels_3state, weekly_labels_12state, state_names)
...
def main():
    ...
    # 创建打标器
    labeler = HS300Labeler(data_dir=str(get_tushare_root()), output_dir="images/label")
