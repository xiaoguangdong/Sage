...
class MacroPredictionRunner:
    ...
    def __init__(
        self, data_dir: str = None, processed_dir: str = None, config_path: str = "config/sw_nbs_mapping.yaml"
    ):
        """
        初始化运行器
    ...
    def predict_daily(self, start_date: str, end_date: str, output_file: Optional[str] = None) -> List[Dict]:
        """
        每日预测
    ...
    def predict_backtest(self, backtest_periods: List[Dict[str, str]]) -> Dict[str, List[Dict]]:
        """
        回测预测
        ...
        for period in backtest_periods:
            ...
            results = self.predict_daily(start_date=period["start"], end_date=period["end"])
    ...
    def run(
        ...
        if mode == "daily":
            ...
            # 定义回测周期（3个牛市阶段）
            backtest_periods = [
                {"start": "2024-09-01", "end": "2025-01-01", "name": "牛市阶段1"},
                {"start": "2024-02-01", "end": "2024-04-01", "name": "牛市阶段2"},
                {"start": "2020-07-01", "end": "2020-09-01", "name": "牛市阶段3"},
            ]
...
def main():
    ...
    parser.add_argument("--data-dir", type=str, default=str(get_tushare_root() / "macro"), help="数据目录")
    parser.add_argument("--processed-dir", type=str, default=str(get_data_path("processed")), help="处理后数据目录")
    parser.add_argument("--config", type=str, default="config/sw_nbs_mapping.yaml", help="配置文件路径")

    args = parser.parse_args()

    # 创建运行器
    runner = MacroPredictionRunner(data_dir=args.data_dir, processed_dir=args.processed_dir, config_path=args.config)

    # 运行预测
    runner.run(mode=args.mode, start_date=args.start_date, end_date=args.end_date, output_file=args.output)
