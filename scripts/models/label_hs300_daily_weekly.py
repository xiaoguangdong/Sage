...
class HS300Labeler:
    ...
    def load_stock_data(self):
        ...
        if daily_files:
            ...
            # 筛选沪深300成分股
            self.df_daily = self.df_daily[self.df_daily["ts_code"].isin(self.stock_list)]
    ...
    def calculate_breadth(self):
        ...
        # 计算上涨股票占比
        weekly_rising = (
            self.df_daily.groupby(pd.Grouper(key="trade_date", freq=freq))
        ...
        # 合并到主数据
        self.df = pd.merge(self.df, weekly_rising, on="date", how="left")

        self.df = pd.merge(self.df, weekly_new_high, on="date", how="left")
    ...
    def label_original_smooth(self):
        ...
        print(
            f"✓ 原始标签平滑完成（method={self.smooth_method}, "
    ...
    def visualize(self):
        ...
        for name, col, ax in methods:
            ...
            for i in range(1, len(df)):
                if df[col].iloc[i] == 2:
                    ax.axvspan(df["date"].iloc[i - 1], df["date"].iloc[i], alpha=0.2, color="green")
                elif df[col].iloc[i] == 0:
                    ax.axvspan(df["date"].iloc[i - 1], df["date"].iloc[i], alpha=0.2, color="red")
