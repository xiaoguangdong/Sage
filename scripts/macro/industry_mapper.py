...
class IndustryMapper:
    ...
    def fuzzy_match_nbs(self, nbs_name: str, nbs_data: pd.DataFrame, nbs_name_col: str = 'industry', threshold: float = 0.6) -> Optional[str]:
        """
        模糊匹配NBS行业名称
    ...
    def map_nbs_data_to_sw(
        ...
        value_col: str = 'ppi_yoy',
        date_col: str = 'date',
        nbs_name_col: str = 'industry'
    ) -> pd.DataFrame:
        """
        将NBS宏观数据映射到申万行业
        ...
        for sw_industry, nbs_mappings in self.sw_to_nbs_map.items():
            ...
            if abs(total_weight - 1.0) > 0.01:  # 允许1%的误差
                results['warnings'].append(
                    f"{sw_industry}: 权重总和为{total_weight:.2f}，不等于1.0"
                )
