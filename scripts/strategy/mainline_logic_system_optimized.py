...
from scripts.data._shared.runtime import get_data_path, get_project_root, get_tushare_root
...
class MainlineLogicSystemOptimized:
    ...
    def calculate_sector_leadership(self, sector_stocks, date):
        ...
        """
        # 获取板块内股票在指定日期的数据
        sector_data = self.daily[(self.daily["ts_code"].isin(sector_stocks)) & (self.daily["trade_date"] == date)]
        ...
        index_data = self.index_data[
            (self.index_data["trade_date"] == date) & (self.index_data["index_name"].isin(["沪深300", "中证500"]))
        ]
        if len(index_data) > 0:
    ...
    def calculate_sector_confirmation(self, sector_stocks, date):
        ...
        # 综合得分
        confirmation_score = (
            profit_improve_ratio * 100 * 0.5 + roe_improve_ratio * 100 * 0.3 + ocf_improve_ratio * 100 * 0.2
        )
    ...
    def calculate_sector_crowding(self, sector_stocks, date):
        ...
        sector_basic = self.daily_basic[
            (self.daily_basic["ts_code"].isin(sector_stocks)) & (self.daily_basic["trade_date"] == date)
        ]
    ...
    def rank_sectors(self, date, top_k=3):
        ...
        for idx, row in top_sectors.iterrows():
            print(
                f"    {idx+1}. {row['sector_name']}: {row['total_score']:.2f} "
    ...
    def calculate_stock_growth_score(self, ts_code, date):
        ...
        """
        # 获取个股数据
        stock_data = self.daily[(self.daily["ts_code"] == ts_code) & (self.daily["trade_date"] <= date)].tail(
            60
        )  # 最近60天
        ...
        # 综合得分
        growth_score = ret_4w * 0.4 + ret_12w * 0.4 + ret_acc * 0.2
    ...
    def calculate_stock_quality_score(self, ts_code, date):
        ...
        stock_fina = self.fina_indicator[
            (self.fina_indicator["ts_code"] == ts_code) & (self.fina_indicator["end_date"] <= pd.to_datetime(date))
        ]
        ...
        # 综合得分
        quality_score = (roe * 2) + (gross_margin * 2) + ((1 - debt_to_assets) * 100)
    ...
    def calculate_stock_price_score(self, ts_code, date):
        ...
        """
        # 获取个股数据
        stock_data = self.daily[(self.daily["ts_code"] == ts_code) & (self.daily["trade_date"] <= date)].tail(60)
        ...
        # 综合得分
        price_score = breakout * 30 + drawdown_score * 0.3 + sharpe * 10
    ...
    def calculate_stock_liquidity_score(self, ts_code, date):
        ...
        stock_basic = self.daily_basic[
            (self.daily_basic["ts_code"] == ts_code) & (self.daily_basic["trade_date"] == date)
        ]
    ...
    def select_stocks_in_sectors(self, top_sectors, date, stocks_per_sector=5):
        ...
        for idx, sector_row in top_sectors.iterrows():
            ...
            if self.industry_members is not None and sector_name in self.industry_members["industry_name"].values:
                stocks = self.industry_members[self.industry_members["industry_name"] == sector_name][
                    "con_code"
                ].tolist()
            # 从概念成分股获取
            ...
            for ts_code in stocks:
                ...
                stock_scores.append({"ts_code": ts_code, "score": score, "sector": sector_name})
...
if __name__ == "__main__":
    ...
    # 运行系统（测试一个日期）
    results = system.run(start_date="2024-06-01", end_date="2024-06-01", frequency="D")
