...
class MainlineLogicSystem:
    ...
    def load_sector_data(self):
        ...
        if os.path.exists(industry_members_file) and os.path.exists(industry_l1_file):
            ...
            self.industry_members = members.merge(
                l1[["index_code", "industry_name"]], on="index_code", how="left"
            ).dropna(subset=["industry_name", "ts_code"])
            print(f"  ✓ 行业成分股: {len(self.industry_members)} 条记录")
        else:
            print(
                "  ! 行业成分股数据不存在，请先运行 tushare_downloader.py --task sw_industry_classify + sw_index_member"
            )
        ...
        if os.path.exists(industry_bias_path):
            try:
                ...
                if not industry_bias.empty:
                    ...
                    latest_day = (
                        industry_bias["trade_date"].max()
                        if pd.to_datetime(date) > industry_bias["trade_date"].max()
    ...
    def calculate_sector_diffusion(self, sector_stocks, date):
        ...
        sector_data = self.daily[
            (self.daily["ts_code"].isin(sector_stocks)) & (self.daily["trade_date"] <= date)
    ...
    def calculate_sector_confirmation(self, sector_stocks, date):
        ...
        sector_fina = self.fina_indicator[
            (self.fina_indicator["ts_code"].isin(sector_stocks)) & (self.fina_indicator["end_date"] <= date)
        ]
    ...
    def calculate_stock_score(self, ts_code, date):
        ...
        """
        # 获取个股数据
        stock_data = self.daily[(self.daily["ts_code"] == ts_code) & (self.daily["trade_date"] <= date)].sort_values(
            "trade_date"
        )
...
if __name__ == "__main__":
    ...
    # 运行系统
    results = system.run(start_date="2024-01-01", end_date="2024-12-31", frequency="W")
