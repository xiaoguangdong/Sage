...
class FAIToSWAligner:
    ...
    def __init__(self, data_dir: str = None):
        ...
        for sw_industry, nbs_list in self.sw_to_nbs.items():
            for nbs_item in nbs_list:
                ...
                self.nbs_to_sw[nbs_name].append({"sw_industry": sw_industry, "weight": nbs_item["weight"]})
    ...
    def align_fai_to_sw(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(aligned_df) > 0:
            ...
            # 统计每个申万行业的数据点
            sw_coverage = (
                aligned_df.groupby("sw_industry").agg({"fai_name": "nunique", "date": "nunique"}).reset_index()
            )
    ...
    def aggregate_to_sw_level(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 按申万行业和日期分组，计算加权平均
        sw_fai_df = df.groupby(["sw_industry", "date"]).agg({"fai_yoy_weighted": "sum", "weight": "sum"}).reset_index()
    ...
    def process_all(self):
        ...
        return {"aligned": aligned_df, "aggregated": sw_fai_df, "signals": expansion_signals}
