...
class MacroDataProcessor:
    ...
    def load_nbs_industry(self) -> pd.DataFrame:
        ...
        # 加载分行业PPI数据
        ppi_files = ["nbs_ppi_industry_2020.csv", "nbs_ppi_industry_202512.csv"]
        ...
        # 加载分行业FAI数据
        fai_files = ["nbs_fai_industry_2020.csv", "nbs_fai_industry_202512.csv"]
        ...
        # 加载分行业Output数据
        output_files = ["nbs_output_2020.csv", "nbs_output_202512.csv"]
    ...
    def _build_sw_ppi_from_nbs(self) -> pd.DataFrame:
        ...
        # 由环比指数构建链式指数，再计算同比
        df["ppi_index"] = (
            df.groupby("industry_norm")["ppi_mom"]
            .apply(lambda s: (s / 100.0).cumprod() * 100.0)
        ...
        for sw, items in sw_to_nbs.items():
            ...
            grouped = (
                ...
                .apply(lambda g: (g["ppi_yoy"] * g["weight"]).sum() / g["weight"].sum())
    ...
    def load_sw_turnover_rate(self, sw_list: List[str]) -> pd.DataFrame:
        ...
        if "amount" in df.columns and "float_mv" in df.columns:
            df["turnover_rate"] = pd.to_numeric(df["amount"], errors="coerce") / pd.to_numeric(
                df["float_mv"], errors="coerce"
            ).replace(0, np.nan)
    ...
    def load_sw_stock_industry_map(self) -> pd.DataFrame:
        ...
        l2_map = l2[["index_code", "parent_code", "industry_name"]].rename(
            columns={"industry_name": "industry_name_l2"}
        )
    ...
    def load_northbound_features(self) -> pd.DataFrame:
        ...
        df["ratio_ma"] = df.groupby("industry_code")["industry_ratio"].transform(lambda s: s.rolling(window=20).mean())
        df["ratio_std"] = df.groupby("industry_code")["industry_ratio"].transform(lambda s: s.rolling(window=20).std())
    ...
    def clean_industry_data(self, industry_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in numeric_cols:
            if col in df.columns:
                df[f"{col}_ma3"] = df.groupby(group_col)[col].transform(lambda x: x.rolling(3, min_periods=1).mean())
    ...
    def add_market_data(self, industry_df: pd.DataFrame, sw_list: List[str]) -> pd.DataFrame:
        ...
        if industry_df.empty:
            ...
            for frame in frames[1:]:
                base = base.merge(
                    frame[["sw_industry", "date"]].drop_duplicates(), on=["sw_industry", "date"], how="outer"
                )
            result = base
        else:
            ...
            if not rev_yoy_df.empty:
                base = base.merge(
                    rev_yoy_df[["sw_industry", "date"]].drop_duplicates(), on=["sw_industry", "date"], how="outer"
                )
    ...
    def process_all(self) -> Dict[str, pd.DataFrame]:
        ...
        if "output_yoy" in industry_final.columns and "rev_yoy" in industry_final.columns:
            mask = (
                industry_final["inventory_yoy"].isna()
                & industry_final["output_yoy"].notna()
                & industry_final["rev_yoy"].notna()
            )
            industry_final.loc[mask, "inventory_yoy"] = (
                industry_final.loc[mask, "output_yoy"] - industry_final.loc[mask, "rev_yoy"]
            )
        ...
        if proxy_col and "output_yoy" in industry_final.columns:
            mask = (
                industry_final["inventory_yoy"].isna()
                & industry_final["output_yoy"].notna()
                & industry_final[proxy_col].notna()
            )
            industry_final.loc[mask, "inventory_yoy"] = (
                industry_final.loc[mask, "output_yoy"] - industry_final.loc[mask, proxy_col]
            )
        ...
        if "inventory_yoy" in industry_final.columns and "output_yoy" in industry_final.columns:
            ...
            filled_mask = (
                before_ffill.isna() & industry_final["inventory_yoy"].notna() & industry_final["output_yoy"].notna()
            )
        ...
        return {"macro": macro_cleaned, "industry": industry_final, "northbound": northbound}
...
def main():
    ...
    if "industry_scope" in data["industry"].columns:
        ...
        for col in industrial_only_cols:
            if col in data["industry"].columns:
                eligible_null[col] = (
                    float(data["industry"].loc[industrial_mask, col].isna().mean()) if industrial_mask.any() else 1.0
                )
    ...
    report = {
        ...
        "northbound": _coverage(
            data["northbound"], ["sw_industry", "trade_date", "northbound_signal", "industry_ratio"]
        ),
        "causes": {
            "macro": {
