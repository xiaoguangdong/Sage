...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/macro/map_nbs_to_sw_all.log", encoding="utf-8"), logging.StreamHandler()],
)
...
class NBSMapperAll:
    ...
    def process(self, nbs_file: str = None, value_col: str = "ppi_yoy") -> tuple:
        ...
        # 4. 保存数据
        if len(sw_l1_data) > 0:
            self.save_sw_data(sw_l1_data, level="L1", value_col=value_col)

        if len(sw_l2_data) > 0:
            self.save_sw_data(sw_l2_data, level="L2", value_col=value_col)
