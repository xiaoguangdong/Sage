...
def get_sw_valuation_data(start_date, end_date):
    ...
    ).filter(finance.SW1_DAILY_VALUATION.date >= start_date, finance.SW1_DAILY_VALUATION.date < end_date)
...
def calculate_percentile(df, period_days=252 * 3):
    ...
    for code, group in df.groupby("code"):
        ...
        for i in range(len(group)):
            ...
            historical_data = group[(group["date"] >= historical_start) & (group["date"] < current_date)]
            ...
            result_list.append(
                {"code": code, "date": current_date, "pb_percentile": pb_percentile, "pe_percentile": pe_percentile}
            )
...
def main():
    ...
    try:
        ...
        print(
            latest_data[["industry_name", "pe", "pb", "pb_percentile", "pe_percentile"]].head(10).to_string(index=False)
        )
