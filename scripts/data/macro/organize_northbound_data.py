...
def merge_data(constituents_df, holdings_df):
    ...
    merged = holdings_df.merge(
        constituents_df[["con_code", "index_code", "weight"]], left_on="ts_code", right_on="con_code", how="inner"
    )
...
def calculate_northbound_flow(merged_df):
    ...
    # 按日期聚合
    daily_flow = merged_df.groupby("trade_date").agg({"vol_change": "sum", "vol": "sum", "ratio": "mean"}).reset_index()

    daily_flow = daily_flow.rename(columns={"vol_change": "net_flow", "vol": "total_holdings", "ratio": "avg_ratio"})
...
def calculate_by_stock(merged_df):
    ...
    # 按股票聚合
    stock_flow = (
        merged_df.groupby("ts_code")
        .agg({"vol_change": "sum", "vol": "last", "name": "last", "weight": "last"})
        .reset_index()
    )

    stock_flow = stock_flow.rename(columns={"vol_change": "net_flow", "vol": "total_holdings"})
