...
def get_sw_industry_constituents():
    ...
    merged = members.merge(
        (
            industries[["index_code", "industry_name", "industry_code"]]
            if "industry_code" in industries.columns
            else industries[["index_code", "industry_name"]]
        ),
        on="index_code",
        how="left",
    )
...
def calculate_industry_northbound_flow(holdings_df, constituents_df):
    ...
    # 合并持仓数据和成分股数据
    merged = holdings_df.merge(constituents_df[["ts_code", "industry_code", "industry_name"]], on="ts_code", how="left")
    ...
    # 按行业和日期聚合
    industry_flow = (
        merged.groupby(["industry_code", "industry_name", "trade_date"])
        .agg({"vol": "sum", "ratio": "mean"})
        .reset_index()
    )
